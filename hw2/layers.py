...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dx = dout.clone()
        dx = torch.where(x > 0, dx, self.alpha * dx)
        # ========================
...
class Linear(Layer):
    ...
    def backward(self, dout):
        ...
        d = dout.clone()
        gradx = self.w
        dx = torch.matmul(d, gradx)
        gradw = x.clone()
        dw = torch.matmul(d.T, gradw)
        db = torch.sum(d, dim=0)
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        correct_class_score = -x[torch.arange(N), y]
        loss = torch.mean(correct_class_score + torch.log(torch.sum(torch.exp(x), dim=1)))
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        cs[torch.arange(N), y] = -1

        grad = (cs + torch.exp(x) / torch.sum(torch.exp(x), dim=1, keepdim=True))
        dx = dout * grad / N
...
class Sequential(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        din = dout
        # need to inverse the list to backward from the last
